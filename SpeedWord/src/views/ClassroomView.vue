...
// ============ 全屏课堂 ============
// 动画完成一律由 transitionend/animationend 事件驱动（waitForAnim），
// 绝不使用 setTimeout 猜测动画时长；setTimeout 仅作事件失效时的安全阀，
// 防止极端情况下课堂卡死（不是动画计时来源）。
...
const isRevealed = computed(() =>
  isChoice.value
    ? !!choicePicked.value
    : ["ANSWER_REVEALING", "ANSWER_VISIBLE", "FEEDBACK", "QUESTION_TRANSITIONING"].includes(phase.value)
);
...
async function advance(): Promise<void> {
  ...
  try {
    ...
    classroom.unlock();
    classroom.act("NEXT_DONE");                    // → QUESTION_READY（加载新题）或 FINISHED
...
async function exitClassroom(): Promise<void> {
  ...
  exiting.value = true;
  stopSpeak();
  if (finished.value) { ui.go("game-center"); exiting.value = false; return; }
  ...
  <div class="classroom">
    ...
    <header class="cr-header">
      ...
      <div class="cr-right">
        ...
        <button class="cr-btn" @click="exitClassroom" title="退出 (Esc)">✕ 退出</button>
      </div>
    </header>

    <!-- 完成总结 -->
    <div v-if="finished" class="cr-summary">
      <h2 class="sum-title">🎉 课堂完成！</h2>
        ...
        <div class="card cr-card" :class="{ 'anim-out': transitioning }" ref="cardRef">
          <div class="card-body" :key="q.item.id">
            ...
            <div v-else-if="q.kind === 'context'" class="prompt">
              ...
              <div v-if="q.contextHint" class="context-hint">💡 {{ q.contextHint }}</div>
            </div>
...
.cr-header {
  ...
  padding: 12px 20px; gap: 12px; flex-shrink: 0;
...
.cr-pill {
  ...
  padding: 6px 14px; font-size: 14px; color: var(--text-dim);
}
...
.combo-pill {
  background: var(--warn-soft); color: #ffe3ab; border: 1px solid var(--warn);
...
/* ---------- 舞台 ---------- */
.cr-stage { flex: 1; display: flex; align-items: center; justify-content: center; padding: 20px; min-height: 0; }
.cr-card {
  width: min(880px, 94%); min-height: 320px;
...
@keyframes card-out { from { opacity: 1; transform: none; } to { opacity: 0; transform: translateY(-16px) scale(0.98); } }

/* ---------- 线索 ---------- */
.prompt { display: flex; flex-direction: column; align-items: center; gap: 14px; text-align: center; }
.prompt-text {
  font-size: clamp(34px, 7vw, 64px); font-weight: 800; letter-spacing: 0.5px;
  word-break: break-word; line-height: 1.2;
}
.prompt-img {
  max-width: min(360px, 70vw); max-height: 300px; border-radius: 18px;
...
.opt {
  ...
  padding: 12px 18px; font-size: 20px; transition: transform 0.08s;
}
...
.opt-key {
  ...
  background: var(--card-2); font-weight: 800; font-size: 18px; color: var(--text-dim);
}
...
/* ---------- 答案 ---------- */
.answer { margin-top: 26px; border-top: 1px dashed var(--line); padding-top: 20px; text-align: center; animation: answer-in 0.35s ease; }
@keyframes answer-in { from { opacity: 0; transform: translateY(14px); } to { opacity: 1; transform: none; } }
...
/* ---------- 底部 ---------- */
.cr-footer { padding: 18px 24px 26px; flex-shrink: 0; }
.footer-center { display: flex; justify-content: center; }
.reveal-btn { min-width: 320px; min-height: 88px; font-size: 22px; animation: pulse 1.8s infinite; }
.feedback-bar { display: flex; gap: 12px; justify-content: center; flex-wrap: wrap; }
.fb-btn, .fb-next {
  display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 4px;
  min-width: 150px; min-height: 84px; border-radius: 18px; font-weight: 700;
  background: var(--card-2); border: 1px solid var(--line); padding: 10px 16px;
}
.fb-btn:hover, .fb-next:hover { border-color: var(--text-faint); }
...
/* ---------- 总结 ---------- */
.cr-summary { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 6px; padding: 30px; text-align: center; animation: card-in 0.4s ease; }
